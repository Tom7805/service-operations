...
import { SESSION_IDLE_MINUTES } from '../../../hooks/useIdleLogout';
...
interface LoginFormProps {
  ...
  /** NCL-01-CN-001-TC-03: lý do phiên trước bị kết thúc (hết hạn / không thao tác). */
  notice?: string | null;
}
...
export default function LoginForm({ onAuthenticated, onForgotPassword, onTwoFactorRequired, notice }: LoginFormProps) {
  ...
  return <div className="login-card">
    ...
    {notice && !error && <p className="form-error form-notice" role="status" data-testid="login-session-notice"><span>i</span>{notice}</p>}
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className="form-options"><span /><a href="#forgot-password" onClick={(event) => { event.preventDefault(); onForgotPassword?.(); }}>Quên mật khẩu?</a></div>
