...
/**
 * Báo cáo biên lợi nhuận theo khách hàng (NCL-09-CN-005, TC-01).
 *
 * Gộp doanh thu ghi nhận và giá vốn giờ công của mọi dòng giờ công đã duyệt (`APPROVED`) có
 * `workDate` trong kỳ `from`..`to`, theo từng khách hàng (qua các dự án của khách hàng đó).
 *
 * Chỉ Ban giám đốc (VT-01) được xem báo cáo này — vai trò khác nhận `403 FORBIDDEN`.
 */
export default function MarginByCustomerPage({
  currentUserRoles = ['VT-01'],
  onBack,
}: MarginByCustomerPageProps) {
  ...
  const [report, setReport] = useState<MarginByCustomerRes | null>(null);
  ...
  const loadReport = useCallback(async (from: string, to: string) => {
    ...
    setError(null);
    try {
      const data = await getMarginByCustomer(from, to);
      setReport(data);
      setHasSearched(true);
    } catch (err) {
      const message =
        err instanceof ProfitabilityApiError || err instanceof Error
          ? err.message
          : 'Không thể tải báo cáo biên lợi nhuận theo khách hàng.';
      setError(message);
      setReport(null);
    } finally {
      setLoading(false);
        ...
        <div className="toolbar-filters" style={{ display: 'flex', alignItems: 'flex-end', gap: '16px', flexWrap: 'wrap' }}>
          <div className="filter-group">
            ...
            <input
              ...
              disabled={loading}
              data-testid="margin-customer-from-input"
            />
          </div>
          <div className="filter-group">
            <label className="filter-label" htmlFor="margin-customer-to">
            ...
            <input
              ...
              disabled={loading}
              data-testid="margin-customer-to-input"
            />
          </div>
          <button type="submit" className="btn btn-primary btn-sm" disabled={loading} data-testid="btn-view-margin-by-customer">
            {ICONS.search} {loading ? 'Đang tải...' : 'Xem báo cáo'}
          </button>
        </div>
        {fieldError && (
          <p className="field-error" data-testid="error-margin-by-customer" style={{ color: '#DC2626', fontSize: '13px', marginTop: '8px' }}>
            {fieldError}
          </p>
        )}
      </form>
          ...
          <>
            ...
            <div className="user-table-card" style={{ padding: '20px' }} data-testid="margin-by-customer-table">
              <div style={{ marginBottom: '16px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#1E293B' }}>Chi tiết theo khách hàng</h3>
              </div>

              {report.lines.length === 0 ? (
                <p style={{ margin: 0, color: '#64748B', fontSize: '13.5px' }} data-testid="margin-by-customer-empty">
                    ...
                    <tbody>
                      {report.lines.map((line) => (
                        <tr key={line.customerId} data-testid={`margin-by-customer-line-${line.customerId}`}>
                          <td>
                            {line.customerName}
                            <div style={{ color: '#94A3B8', fontSize: '12.5px' }}>{line.customerCode}</div>
                          </td>
