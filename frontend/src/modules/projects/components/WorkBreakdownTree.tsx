...
import type { TaskRes, WorkBreakdownRes } from '../types/taskTypes';
...
function buildTaskTree(tasks: TaskRes[]): TaskNode[] {
  ...
  tasks.forEach((t) => {
    ...
    if (t.parentTaskId != null && map.has(t.parentTaskId)) {
...
export default function WorkBreakdownTree({
  ...
  const renderTaskNode = (node: TaskNode, wp: WorkBreakdownRes, level = 0) => {
    ...
    const badge = statusBadgeConfig[task.status] || {
      label: task.status,
      className: 'wbs-badge--todo',
    };

    return (
      <div
        key={task.id}
        className={`wbs-task-row ${level > 0 ? 'wbs-task-row--nested' : ''}`}
        style={{ marginLeft: `${level * 20}px` }}
        data-testid={`task-row-${task.id}`}
      >
        <div className="wbs-task-main">
          <span className="wbs-task-title">
            <span className="wbs-task-bullet">•</span> {task.name}
          </span>
          <div className="wbs-task-meta">
            {task.expectedStartDate && (
              <span>
                Thời hạn: {formatDate(task.expectedStartDate)}
                {task.expectedEndDate && ` ➔ ${formatDate(task.expectedEndDate)}`}
              </span>
            )}
            ...
            {task.description && <span className="wbs-task-desc-inline">{task.description}</span>}
          </div>
        </div>
      ...
      >
        ...
        {!isClosed && (
          <div className="wbs-package-body">
            ...
            {(!wp.tasks || wp.tasks.length === 0) && (
              <div className="wbs-empty-hint">Chưa có công việc trong hạng mục này.</div>
            )}

            {wp.children && wp.children.length > 0 && (
              <div className="wbs-subpackages">
  ...
  return (
    <div className="wbs-tree" data-testid="wbs-tree">
      {items.map((wp) => renderPackageNode(wp, 0))}
    </div>
  );
}
