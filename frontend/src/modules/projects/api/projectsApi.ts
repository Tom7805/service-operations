...
/**
 * Danh sách tài khoản đang ACTIVE để chọn "Người quản lý dự án" (thay vì gõ tay ID).
 * GET /users/assignable-for-project — VT-02 gọi được (VT-07 cũng gọi được).
 */
export async function fetchAssignableProjectManagers(): Promise<AssignableProjectManager[]> {
  return requestBackend<AssignableProjectManager[]>(`${API_BASE_URL}/users/assignable-for-project`, {
    method: 'GET',
  });
}
