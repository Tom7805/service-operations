 ...
 * suy ra nhóm nào thiếu — chỉ việc render đúng những gì trả về.
 */
            ...
            <div key={bucket.bucket} className="user-table-card" style={{ marginBottom: '16px' }}>
              <div className="page-header" style={{ padding: '16px 16px 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div className={`stat-card__icon ${BUCKET_TONE[bucket.bucket] ?? 'stat-card__icon--gray'}`}>
                    {ICONS.clock}
                  </div>
                  <div>
              ...
              {bucket.invoices.length === 0 ? (
                <div className="table-empty-state" data-testid={`aging-empty-${bucket.bucket}`}>
                      ...
                      {bucket.invoices.map((inv) => (
                        <tr key={inv.id}>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            <button type="button" className="btn-icon-refresh" onClick={() => onOpenInvoice(inv.id)} aria-label={`Xem hóa đơn ${inv.invoiceCode}`}>
                              {ICONS.eye}
                            </button>
                          </td>
                        </tr>
