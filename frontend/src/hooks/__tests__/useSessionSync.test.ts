...
vi.mock("../../modules/auth/api/authApi", async () => {
  const actual = await vi.importActual<
    typeof import("../../modules/auth/api/authApi")
...
describe("useSessionSync (NCL-01-CN-004 TC-03)", () => {
  ...
  it("làm mới phiên khi máy chủ trả về vai trò khác", async () => {
    ...
    renderHook(() =>
      useSessionSync({ session, onRefresh, onExpired: vi.fn() }),
    );
  ...
  it("gọi onExpired khi máy chủ trả 401", async () => {
    vi.mocked(authApi.fetchCurrentUser).mockRejectedValue(
      new authApi.SessionSyncError(401, "hết hạn"),
    );
    const onExpired = vi.fn();

    renderHook(() =>
      useSessionSync({ session, onRefresh: vi.fn(), onExpired }),
    );
  ...
  it("không làm gì khi chưa đăng nhập (session = null)", async () => {
    renderHook(() =>
      useSessionSync({ session: null, onRefresh: vi.fn(), onExpired: vi.fn() }),
    );
