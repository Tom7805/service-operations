...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    icon: ICONS.users,
    label: "Nhân sự",
    matches: ["EMPLOYEE_DETAIL"],
    requires: ["VT-06", "VT-07"],
  },
  {
    tab: "PERMISSIONS",
    icon: ICONS.shield,
...
const SYSTEM_NAV_ITEMS: NavItem[] = [
  {
    tab: "TWO_FACTOR_SETTINGS",
    icon: ICONS.key,
    label: "2FA",
    requires: ["VT-07"],
  },
  {
    tab: "SYSTEM_AUDIT_LOG",
    icon: ICONS.history,
...
export default function App() {
  ...
  const [notifTab, setNotifTab] = useState<"ALL" | "MENTIONS" | "SYSTEM">(
    "ALL",
  );
  ...
  const renderNavGroup = (items: NavItem[]) =>
    items.map((item) => {
      const isActive =
        activeTab === item.tab || (item.matches ?? []).includes(activeTab);
      ...
      return (
        ...
        >
          ...
          {!sidebarCollapsed && (
            <span className="side-nav__item__label">{item.label}</span>
          )}
          {!sidebarCollapsed && locked && (
            <span
              className="side-nav__item__lock"
              aria-label="Cần vai trò khác"
            >
              {ICONS.lock}
            </span>
          )}
  ...
  return (
    <div className="app-frame">
      ...
      <div className="app-shell">
        <aside
          className={`side-nav ${sidebarCollapsed ? "side-nav--collapsed" : ""}`}
        >
          ...
          <nav className="side-nav__list" aria-label="Điều hướng chính">
            ...
            <div className="side-nav__group-label">
              {!sidebarCollapsed ? "Bảo mật & Hệ thống" : ""}
            </div>
          ...
          <header className="app-topbar">
            <div className="app-topbar__brand">
              <h1 className="app-topbar__title">
                {activeNavItem?.label ?? "Vận hành dịch vụ"}
              </h1>
            </div>
            ...
            <div className="app-topbar__actions">
                ...
                {notifOpen && (
                  <div className="notif-panel" role="menu">
                    <div className="notif-panel__tabs">
                      ...
                      <span className="notif-panel__chevron">
                        {ICONS.chevronDown}
                      </span>
                    </div>
              ...
              <div className="user-chip" ref={userMenuRef}>
                ...
                >
                  ...
                  <span className="user-chip__chevron">
                    {ICONS.chevronDown}
                  </span>
                </button>
