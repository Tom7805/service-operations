...
| `RESOURCE_NOT_FOUND` | 404 | Không tìm thấy bản ghi hoặc vai trò | `NCL-01-CN-002` và các API tra cứu theo id, ví dụ `NCL-03-CN-006` |
| `INVALID_STATE` | 400 | Thao tác chuyển trạng thái không hợp lệ, hoặc vi phạm ràng buộc nghiệp vụ (ví dụ ngày kết thúc sớm hơn ngày bắt đầu, hoặc thêm hoạt động chăm sóc vào cơ hội đã đóng) | `NCL-01-CN-002`, `NCL-01-CN-003`, `NCL-01-CN-007`, `NCL-03-CN-006` |
| `FORBIDDEN` | 403 | Có tài khoản hợp lệ nhưng không đủ quyền truy cập dữ liệu/chức năng | Các story áp dụng `QTN-01` (phân quyền theo vai trò và cây tổ chức) và `QTN-03` — ví dụ `NCL-01-CN-002`, `NCL-01-CN-006`, `NCL-02-CN-003`, `NCL-03-CN-006` |
