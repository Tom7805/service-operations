...
| `VALIDATION_ERROR` | 400 | Dữ liệu đầu vào không hợp lệ — xem thêm mảng `fieldErrors` trong response khi lỗi đến từ body `@Valid`; API chỉ dùng `@RequestParam` (vd `GET /me/time-entries`) trả cùng `errorCode` nhưng không có `fieldErrors`, chỉ có `message` mô tả tham số bị thiếu/sai định dạng | Toàn bộ API có `@Valid` hoặc `@RequestParam` bắt buộc |
| `RESOURCE_NOT_FOUND` | 404 | Không tìm thấy bản ghi hoặc vai trò (hoặc dữ liệu kế hoạch cần thiết chưa tồn tại — ví dụ dự án chưa gắn báo giá nào ở `NCL-09-CN-006`) | `NCL-01-CN-002` và các API tra cứu theo id, ví dụ `NCL-03-CN-006`, `NCL-06-CN-001`, `NCL-06-CN-003`, `NCL-09-CN-006` |
| `INVALID_STATE` | 400 | Thao tác chuyển trạng thái không hợp lệ, hoặc vi phạm ràng buộc nghiệp vụ (ví dụ ngày kết thúc sớm hơn ngày bắt đầu, thêm hoạt động chăm sóc vào cơ hội đã đóng, ghi giờ công cho dự án đã đóng/ngày ở tương lai, nộp bảng chấm công khi có ngày vượt 12 giờ, hoặc duyệt bảng chấm công sai trạng thái) | `NCL-01-CN-002`, `NCL-01-CN-003`, `NCL-01-CN-007`, `NCL-03-CN-006`, `NCL-06-CN-001`, `NCL-06-CN-002`, `NCL-06-CN-003` |
| `FORBIDDEN` | 403 | Có tài khoản hợp lệ nhưng không đủ quyền truy cập dữ liệu/chức năng | Các story áp dụng `QTN-01` (phân quyền theo vai trò và cây tổ chức) và `QTN-03` — ví dụ `NCL-01-CN-002`, `NCL-01-CN-006`, `NCL-02-CN-003`, `NCL-03-CN-006`, `NCL-06-CN-001`, `NCL-09-CN-005`, `NCL-09-CN-006` |
