...
import com.serviceops.modules.opportunity.logging.OpportunityAccessDeniedAspect;
...
/**
 * Unit test OpportunityAccessDeniedAspect (NCL-03-CN-001, TC-03): ghi nhat ky lan tu
 * choi truy cap ma khong lam hong response 403 goc, ke ca khi chinh viec ghi log bi loi.
 */
@ExtendWith(MockitoExtension.class)
class OpportunityAccessDeniedAspectTest {
	...
	private OpportunityAccessDeniedAspect aspect;

	@BeforeEach
	void setUp() {
		aspect = new OpportunityAccessDeniedAspect(logger);
		lenient().when(joinPoint.getSignature()).thenReturn(signature);
		lenient().when(signature.toShortString()).thenReturn("OpportunityController.create()");
	}
